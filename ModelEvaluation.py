...
if __name__ == '__main__':
    ...
    data.append(['Okapi',1.2, .75, 500,'NA','NA','NA', round(ev.map(),2), round(ndcg,2)])
    ...
    with open('CampusWireHeading_Queries.txt') as query_file:
        for query_num, line in enumerate(query_file):
            ...
            ndcg += ev.ndcg(results, query_start + query_num, top_k)
    ndcg= ndcg / num_queries
    data.append(['Okapi',1.3, .8, 500,'NA','NA','NA', round(ev2.map(),2),round(ndcg,2)])
    ...
    with open('CampusWireHeading_Queries.txt') as query_file:
        for query_num, line in enumerate(query_file):
            ...
            ndcg += ev.ndcg(results, query_start + query_num, top_k)
    ndcg= ndcg / num_queries
    data.append(['Okapi',1.1, .5, 500,'NA','NA','NA', round(ev3.map(),2),round(ndcg,2)])
    ...
    with open('CampusWireHeading_Queries.txt') as query_file:
        for query_num, line in enumerate(query_file):
            ...
            ndcg += ev.ndcg(results, query_start + query_num, top_k)
    ndcg= ndcg / num_queries
    data.append(['Pivoted Length','NA','NA','NA',.2, 'NA','NA', round(ev4.map(),2), round(ndcg,2)])
    ...
    with open('CampusWireHeading_Queries.txt') as query_file:
        for query_num, line in enumerate(query_file):
            ...
            ndcg += ev.ndcg(results, query_start + query_num, top_k)
    ndcg= ndcg / num_queries
    data.append(['Pivoted Length','NA','NA','NA',.01, 'NA','NA', round(ev5.map(),2),round(ndcg,2)])
    ...
    with open('CampusWireHeading_Queries.txt') as query_file:
        for query_num, line in enumerate(query_file):
            ...
            ndcg += ev.ndcg(results, query_start + query_num, top_k)
    ndcg= ndcg / num_queries
    data.append(['Pivoted Length','NA','NA','NA',.5, 'NA','NA', round(evhi.map(),2),round(ndcg,2)])
    ...
    with open('CampusWireHeading_Queries.txt') as query_file:
        for query_num, line in enumerate(query_file):
            ...
            ndcg += ev.ndcg(results, query_start + query_num, top_k)
    ndcg= ndcg / num_queries
    data.append(['JelineK-Mercer','NA','NA','NA','NA', '.7','NA', round(ev6.map(),2),round(ndcg,2)])
    ...
    with open('CampusWireHeading_Queries.txt') as query_file:
        for query_num, line in enumerate(query_file):
            ...
            ndcg += ev.ndcg(results, query_start + query_num, top_k)
    ndcg= ndcg / num_queries
    data.append(['JelineK-Mercer','NA','NA','NA','NA', '.5','NA', round(ev7.map(),2),round(ndcg,2)])
    ...
    with open('CampusWireHeading_Queries.txt') as query_file:
        for query_num, line in enumerate(query_file):
            ...
            ndcg += ev.ndcg(results, query_start + query_num, top_k)
    ndcg= ndcg / num_queries
    data.append(['JelineK-Mercer','NA','NA','NA','NA', '.85','NA', round(ev8.map(),2),round(ndcg,2)])
    ...
    with open('CampusWireHeading_Queries.txt') as query_file:
        for query_num, line in enumerate(query_file):
            ...
            ndcg += ev.ndcg(results, query_start + query_num, top_k)
    ndcg= ndcg / num_queries
    data.append(['Dirichlet Prior','NA','NA','NA','NA', 'NA','2000', round(ev9.map(),2),round(ndcg,2)])
    ...
    with open('CampusWireHeading_Queries.txt') as query_file:
        for query_num, line in enumerate(query_file):
            ...
            ndcg += ev.ndcg(results, query_start + query_num, top_k)
    ndcg= ndcg / num_queries
    data.append(['Dirichlet Prior','NA','NA','NA','NA', 'NA','1500', round(ev10.map(),2),round(ndcg,2)])
    ...
    with open('CampusWireHeading_Queries.txt') as query_file:
        for query_num, line in enumerate(query_file):
            ...
            ndcg += ev.ndcg(results, query_start + query_num, top_k)
    ndcg= ndcg / num_queries
    data.append(['Dirichlet Prior','NA','NA','NA','NA', 'NA','3500', round(ev11.map(),2),round(ndcg,2)])

    EvaluationDF = pd.DataFrame(data, columns=['Ranker','k1','b','k3','s','lambda','mu','MAP','NDCG'])
    print(EvaluationDF)
